...
def main(args):
    topic = args.topic
    print(
        f"bootstrap_servers = {args.bootstrap_servers}, topic = {args.topic}, group = {args.group}"
    )

    protobuf_deserializer = ProtobufDeserializer(
        user_pb2.User, {"use.deprecated.format": False}
    )
        ...
        try:
            ...
            if user is not None:
                print(
                    ...
                    "\tfavorite_number: {}\n"
                    "\tfavorite_color: {}\n".format(
                        msg.key(), user.name, user.favorite_color, user.favorite_number
                    )
                )
